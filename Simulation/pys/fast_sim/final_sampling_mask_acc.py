...
class EVB_Rv_box:
    ...
    """
    def __init__(self, mask, dust_effect_on_box,  Sampled_map):
        """
        ...
        self.SampledNcount_map_all_z = Sampled_map[:, mask]
...
def all_shell_box(usable_label_all, Rv_EBV_mask_all, sky_mask, dust_effect_all, ExpNcount_maps):
    """
    ...
    for i in tqdm.tqdm(range(len(usable_label_all))):
        ...
        mask_here = Rv_EBV_mask_all[label]
        dust_effect_here = dust_effect_all[label]
        true_healpix = np.where(mask_here == 1)[0]
        box = EVB_Rv_box(mask_here, dust_effect_here, Sampled_Ncount)
